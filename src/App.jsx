...
function fixImageUrl(raw) {
  if (!raw) return "";
  // API stores images as http://localhost:5000/uploads/filename.jpg
  // Replace localhost:5000 with the actual render backend
  return raw.replace("http://localhost:5000", "https://backend-holi-images-1.onrender.com");
}

/* ── Image Card ── */
function ImageCard({ img, index, onClick }) {
  const url     = fixImageUrl(img.image||img.imageUrl||img.url||img.path||"");
  const uname   = img.name||img.uploaderName||img.title||"Krishna Bhakt";
